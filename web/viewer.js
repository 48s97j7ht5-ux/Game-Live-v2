...
import { applyMorph, bindMorph, loadTargets } from "./chest-morph.js?v=c38";
import { createHairStudio, parseObjVerts } from "./hair.js?v=c38";
import { createEyes } from "./eye-wear.js?v=c38";
import { createMakeupStudio } from "./makeup.js?v=c38";
import {
  bodyModel,
  bodyPart,
  bodyParts,
...
async function attachPixelFilter() {
  ...
  if (!box) return;
  try {
    const mod = await import("./pixel-mode.js?v=c38");
...
async function boot() {
  catalog = await loadCatalog(new URL("./parts/manifest.json?v=c38", import.meta.url));
