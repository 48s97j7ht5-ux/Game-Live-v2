...
import { applyBody, bindMorph, loadTargets } from "./chest-morph.js?v=c54";
import { createHairStudio, parseObjVerts } from "./hair.js?v=c54";
import { createEyes } from "./eye-wear.js?v=c54";
import { createMakeupStudio } from "./makeup.js?v=c54";
import { createPoseStudio } from "./pose.js?v=c54";
import {
  bodyModel,
  bodyPart,
  bodyParts,
...
async function attachPixelFilter() {
  ...
  if (!box) return;
  try {
    const mod = await import("./pixel-mode.js?v=c54");
...
async function boot() {
  catalog = await loadCatalog(new URL("./parts/manifest.json?v=c54", import.meta.url));
